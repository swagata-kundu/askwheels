...
userModel_Admin.updateUserProfile = function (req, callback) {
  async
    .series([
      ...
      function (cb) {
        if (req.body.email) {
          checkDuplicateRegistratrtion(req.body.email, req.body.contactNo, function (err, status) {
            if (err) {
              return cb(err);
            }
            if (status) {
...
userModel_Admin.sellerDashBoardInfo = function (req, callback) {
  ...
  var sql = "CALL ?? ( ?,?)";
  var parameters = [dbNames.sp.sellerDashBoardInfo, sellerId, subsellerId];
  sql = mysql.format(sql, parameters);
  dbHelper.executeQuery(sql, function (err, result) {
    if (err) {
      return callback(err);
...
userModel_Admin.dealerDashBoardInfo = function (req, callback) {
  var sql = "CALL ?? ()";
  var parameters = [dbNames.sp.dealerDashBoardInfo];
  sql = mysql.format(sql, parameters);
  dbHelper.executeQuery(sql, function (err, result) {
    if (err) {
      return callback(err);
...
userModel_Admin.adminDashBoardInfo = function (req, callback) {
  var sql = "CALL ?? ()";
  var parameters = [dbNames.sp.adminDashBoardInfo];
  sql = mysql.format(sql, parameters);
  dbHelper.executeQuery(sql, function (err, result) {
    if (err) {
      return callback(err);
